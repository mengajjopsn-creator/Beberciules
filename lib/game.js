...
import {startOca,mutateOca,leaveOca,viewOca} from './oca.js';
...
function beginRound(room){
 const available=modes.filter(m=>!['mix','oca'].includes(m.id)&&m.min<=room.players.length);
 const previous=room.round?.type;
 const type=room.selected==='mix'?pick(available.filter(m=>m.id!==previous)).id:room.selected;
...
function finish(room,skipped=false){
 ...
 const finalAnswers=r.stage==='vote'?r.votes:r.answers;
 const values=Object.values(finalAnswers).filter(x=>x!=='pass');
 const ballots=required(room).map(id=>({id,value:finalAnswers[id]??null}));
 let result={skipped};
 if(!skipped){
  ...
  if(['speed','defend'].includes(r.type)){const yes=values.filter(x=>x==='yes').length,no=values.filter(x=>x==='no').length;const success=yes>no;if(success)score(room,r.actor,2);result={yes,no,success,tied:yes===no&&yes>0,noVotes:!values.length};}
  if(!['match','paranoia'].includes(r.type))result.ballots=ballots;
 }
 result.names=Object.fromEntries(room.players.map(p=>[p.id,p.name]));
...
export function mutate(room,player,action,data={}){
 ...
 if(action==='leave'){
  ...
  if(room.oca)leaveOca(room,player.id);
  if(r&&r.stage!=='results'&&room.phase==='playing'){
   ...
   if(r.type==='faker')for(const[id,value]of Object.entries(r.votes))if(value===player.id)r.votes[id]='pass';
   if((['paranoia','bluff','speed','defend'].includes(r.type)&&r.actor===player.id)||r.faker===player.id||activeIds(room).length<modes.find(m=>m.id===r.type).min)finish(room,true);else advanceStage(room);
  }return;
 }
 if(typeof action==='string'&&action.startsWith('oca_'))return mutateOca(room,player,action,data);
 ...
 if(action==='start'){
  ...
  if(room.selected==='oca'){room.players.forEach(p=>p.score=0);startOca(room);return;}
  room.roundNumber=1;room.players.forEach(p=>p.score=0);beginRound(room);return;
 }
 if(action==='lobby'){room.phase='lobby';room.round=null;room.oca=null;return;}
 if(action==='next'){
 ...
 if(action==='done'){
  if(r.stage!=='perform'||(r.actor!==player.id&&!(host&&r.startedAt&&Date.now()>=r.startedAt+(r.type==='speed'?8:15)*1000)))fail('No es tu turno.',403);
...
export function view(room,player){
 ...
 if(room.oca&&['board','board-finished'].includes(room.phase)){result.oca=viewOca(room,player);return result;}
 ...
 if(r.type==='bluff'&&['vote','results'].includes(r.stage))result.round.options=r.options.map(o=>({id:o.id,text:o.text,own:o.authors.includes(player.id),...(isResult?{truth:o.truth,authors:o.authors}: {})}));
 return result;
}
