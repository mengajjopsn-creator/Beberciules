...
export function renderOca(state,selectedCell=null,{zoom=false}={}){
 ...
 const board=`<div class="oca-board-frame ${zoom?'is-zoomed':''}"><div class="oca-board-toolbar"><span>EL TABLERO</span>${button(zoom?'Reducir ↙':'Ampliar ↗','oca_zoom',false,'oca-zoom-button')}</div><div class="oca-board-scroll"><div class="oca-photo-surface"><div class="oca-board" aria-label="Tablero de la oca: salida, 50 pruebas y meta">${g.cells.map((c,i)=>{
  ...
  return `<button type="button" class="oca-cell ${occupants.length?'has-pawns':''} ${walls} oca-tile-${tone(c)} oca-x-${x+1} oca-y-${y+1} ${c.gooseTo?'goose':''} ${c.skip?'penalty':''} ${i===g.positions[g.actor]?'active':''} ${i===51?'finish':''}" data-cell="${i}" aria-label="Casilla ${i}: ${esc(c.title)}${occupants.length?'. '+occupants.map(name).join(', '):''}" aria-pressed="${selected.id===i}"><span class="oca-cell-number">${i===0?'SAL':i}<span aria-hidden="true">${arrow}</span></span><span class="oca-cell-icon" aria-hidden="true">${c.gooseTo?'🪿':icons[c.icon]}</span><span class="oca-occupants">${occupants.slice(0,1).map(token).join('')}${occupants.length>1?`<span class="oca-overflow">+${occupants.length-1}</span>`:''}</span></button>`;
 }).join('')}${road}<div class="oca-board-center"><span>BEBERCIULES</span><strong>LA<br>OCA</strong><small>50 pruebas<br>Una meta</small></div></div></div></div><div class="oca-board-key"><span class="key-kiss">Besos y retos</span><span class="key-drink">Tragos</span><span class="key-travel">Saltos</span><span class="key-rest">Esperas</span></div></div>`;
